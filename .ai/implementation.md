...
- [x] Define flags:
  ...
  - [x] `--lockfile` — reserved; v1 errors with _"--lockfile is not implemented
        in v1; the output dir has no `package-lock.json`. See Phase 8."_
...
- [x] **v1: emit no lockfile.** Document loudly that consumer runs `npm install`
      in output dir.
- [x] `--lockfile` flag is reserved; v1 errors _"--lockfile is not implemented
      in v1."_ (Reserves the flag name so v2 can flip the default without
      flag-name changes.)
- [ ] v2 plan:
...
- [ ] **`npm install` smoke test:**
  ...
  - [ ] After install, run `node -e "require('<pkg-name>')"` to catch broken
        `main`/`exports`
- [x] **CI matrix:** GitHub Actions, Node 20/22/24, Linux + macOS. Windows
...
- [x] `README.md` ordering:
  ...
  5. Limitations — explicitly call out v1 gaps (no lockfile, no `--build`, no
     Windows, no `files` field honored); each item links to its tracking issue
...
- [x] `readonly` fields (resolved to absolute paths where applicable):
  ...
  - [x] `clean: boolean`
  - [x] `lockfile: boolean` — v1 always false; presence of flag triggers an
        early UserError
  - [x] `dryRun: boolean`
  - [x] `verbose: boolean`
...
- [x] `makePackageLockFile(app)` — Phase 8; v1 no-op
...
- **v1 lockfile.** Not generated. `--lockfile` flag reserved and errors in v1;
  v2 lights it up.
